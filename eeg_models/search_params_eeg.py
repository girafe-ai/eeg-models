...
class EegTraining(object):
    ...
    def __init__(self, sampling_rate, dropout_rate, f1, d) -> None:
        self.loss_fn = None
        self.optimizer = None
    ...
    def eeg_model(self, n_samples, dropout_rate, f1, d):
        model = EegNet(2, 16, n_samples, dropout_rate, 128, f1, d)
        return model
    ...
    def set_loaders(
        ...
        f1 = self.f1
        d = self.d
        self.model = self.eeg_model(epoch_count, dropout_rate, f1, d)
        # optimizer
...
def eeg_net_params(dropout_rate=0.5, f1=8, d=2):

    interval_dropout_rate = np.linspace(0.2, 0.8, 7)
    interval_f1 = list(range(7, 25))
    interval_d = list(range(2, 9))

    f1_score_dropout_rate = []
    f1_score_f1 = []
    f1_score_d = []
    # dropout_rate
    for i in interval_dropout_rate:
        dropout_rate = i
        print("dropout_rate = ", dropout_rate, "f1 = ", f1, "d = ", d)
        train_braininvaders = EegTraining(sampling_rate, dropout_rate, f1, d)
        ...
        f1_score_dropout_rate.append(train_braininvaders.results)
    plt.figure(figsize=(15, 15))
    plt.plot(
        interval_dropout_rate,
        f1_score_dropout_rate,
        marker=".",
        label="f1_score_dropout_rate",
    )
    plt.yticks(np.arange(0.2, 0.8, 0.05))
    plt.title("GridSearch for dropout rate")
    plt.xlabel("dropout rate")
    plt.ylabel("f1 score of test")
    plt.legend()
    plt.show()
    ind = np.argmax(f1_score_dropout_rate)
    dropout_rate = interval_dropout_rate[ind]
    print("best dropout rate = ", dropout_rate)
    # f1
    for i in interval_f1:
        f1 = i
        print("dropout_rate = ", dropout_rate, "f1 = ", f1, "d = ", d)
        train_braininvaders = EegTraining(sampling_rate, dropout_rate, f1, d)
        train_braininvaders.set_loaders(
        ...
        train_braininvaders.train_val_test(n_epochs)
        f1_score_f1.append(train_braininvaders.results)
    plt.figure(figsize=(15, 15))
    plt.plot(interval_f1, f1_score_f1, marker=".", label="f1_score_f1")
    plt.yticks(np.arange(0.2, 0.8, 0.05))
    plt.title("GridSearch for f1")
    plt.xlabel("f1")
    plt.ylabel("f1 score of test")
    plt.legend()
    plt.show()
    ind = np.argmax(f1_score_f1)
    f1 = interval_f1[ind]
    print("best f1 = ", f1)
    # d
    for i in interval_d:
        ...
        train_braininvaders.train_val_test(n_epochs)
        f1_score_d.append(train_braininvaders.results)
    plt.figure(figsize=(15, 15))
    plt.plot(interval_d, f1_score_d, marker=".", label="f1_score_d")
    plt.yticks(np.arange(0.2, 0.8, 0.05))
    plt.title("GridSearch for d")
    plt.xlabel("d")
    plt.ylabel("f1 score of test")
    plt.legend()
    plt.show()
    ind = np.argmax(f1_score_d)
    d = interval_d[ind]
    print("best d = ", d)
    print("best parameters are : dropout_rate = ", dropout_rate, "f1 = ", f1, "d = ", d)
