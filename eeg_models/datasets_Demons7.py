...
class DemonsP300Dataset(AbstractEegDataset):
    ...
    def __getitem__(self, index: int) -> Dict[str, Any]:
        ...
        for i in range(len(data)):    # number of sessions (here 1) 
          
            for j in range(len(data[f"session_{i}"])):   #  for each run : 

                data_tmp = []
                label_tmp = []

                signal = data[f"session_{i}"][f"run_{j}"][9]
                signal_test = [(indice, signal[indice]) for indice in range(len(signal)) if signal[indice] == 1] 

                if signal_test == []:
                    for k in range(10):
                        limits = ( 300 + k * 1400, 700 + k * 1400)
                        #print('limits :', k, limits)
                        if data[f"session_{i}"][f"run_{j}"][0:8, current_indice -50 : current_indice + 350 ].shape[1] == 400:
                            data_tmp.append(data[f"session_{i}"][f"run_{j}"][0:8, limits[0] : limits[1]])
                ...
                if signal_test != []:
                    for k in range(len(signal_test)):
                        ...
                        if data[f"session_{i}"][f"run_{j}"][0:8, current_indice -50 : current_indice + 350 ].shape[1] == 400:
                            data_tmp.append( data[f"session_{i}"][f"run_{j}"][0:8, current_indice -50 : current_indice + 350 ])
                            label_tmp.append(np.array([1.]))
            ...
            # 
            data_tab = (np.stack(data_tab))[:170]
            label_tab = (np.stack(label_tab))[:170]
        ...
        output = ( torch.as_tensor(np.stack(filtered_eeg)).float(), 
                torch.as_tensor(np.stack(filtered_label)).float()
                )

        return output   #  tuple of ( ndarray : #_runs, #_channels, #_timestamped_data)
